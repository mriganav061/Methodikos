...
class Session
{
   ...
   function updateprogress($current, $msg) {
   	$filename = $_SERVER['DOCUMENT_ROOT'].'/status1.json';
   	$progress = array('total'=>'100', 'current'=>$current, 'msg'=>$msg);
   	$fp = fopen($filename, "w");
   	fwrite($fp, utf8_encode(json_encode($progress)));
   	fclose($fp);
   	copy($_SERVER['DOCUMENT_ROOT'].'/status1.json', $_SERVER['DOCUMENT_ROOT'].'/status.json');
   }
