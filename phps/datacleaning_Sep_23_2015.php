<?php
	...
	// update
	// update database
	//hpPlayer: only update base year
	for ($k=0 ; $k<count($sections) ; $k++) {
		$year_of_prior_treatment = $sections[$k]['YEAR_OF_PRIOR_TREATMENT'];
		if ($year_of_prior_treatment != "") {
			$query = "UPDATE $table SET PRIOR_TREATMENT='HR', YEAR_OF_PRIOR_TREATMENT='$year_of_prior_treatment' WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']."' 
