<?php
	...
	for ($k=0 ; $k<count($sections) ; $k++) {
		...
		if ($year_of_prior_treatment != "") {
			...
			$query = "SELECT CONDITION_SCORE FROM $table WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']." - 1' 
			...
			if($CS_delta > 40){
				$query = "UPDATE $table SET PRIOR_TREATMENT= 'HR' WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']."' 
				AND SIGNED_HIGHWAY_RDBD_ID = '".$sections[$k]['SIGNED_HIGHWAY_RDBD_ID']."' AND BEG_REF_MARKER_NBR = '".$sections[$k]['BEG_REF_MARKER_NBR']."'
				AND BEG_REF_MARKER_DISP = '".$sections[$k]['BEG_REF_MARKER_DISP']."' AND RATING_CYCLE_CODE = '".$sections[$k]['RATING_CYCLE_CODE']."'";
				mysql_query($query) or die('mysql_error: '.mysql_error());
			}elseif($CS_delta > 30){
				$query = "UPDATE $table SET PRIOR_TREATMENT= 'MR' WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']."' 
				AND SIGNED_HIGHWAY_RDBD_ID = '".$sections[$k]['SIGNED_HIGHWAY_RDBD_ID']."' AND BEG_REF_MARKER_NBR = '".$sections[$k]['BEG_REF_MARKER_NBR']."'
				AND BEG_REF_MARKER_DISP = '".$sections[$k]['BEG_REF_MARKER_DISP']."' AND RATING_CYCLE_CODE = '".$sections[$k]['RATING_CYCLE_CODE']."'";
				mysql_query($query) or die('mysql_error: '.mysql_error());
			}elseif($CS_delta > 15){
				$query = "UPDATE $table SET PRIOR_TREATMENT= 'LR' WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']."' 
				AND SIGNED_HIGHWAY_RDBD_ID = '".$sections[$k]['SIGNED_HIGHWAY_RDBD_ID']."' AND BEG_REF_MARKER_NBR = '".$sections[$k]['BEG_REF_MARKER_NBR']."'
				AND BEG_REF_MARKER_DISP = '".$sections[$k]['BEG_REF_MARKER_DISP']."' AND RATING_CYCLE_CODE = '".$sections[$k]['RATING_CYCLE_CODE']."'";
				mysql_query($query) or die('mysql_error: '.mysql_error());
			}elseif($CS_delta > 5){
				$query = "UPDATE $table SET PRIOR_TREATMENT= 'PM' WHERE FISCAL_YEAR = '".$sections[$k]['FISCAL_YEAR']."' 
				AND SIGNED_HIGHWAY_RDBD_ID = '".$sections[$k]['SIGNED_HIGHWAY_RDBD_ID']."' AND BEG_REF_MARKER_NBR = '".$sections[$k]['BEG_REF_MARKER_NBR']."'
				AND BEG_REF_MARKER_DISP = '".$sections[$k]['BEG_REF_MARKER_DISP']."' AND RATING_CYCLE_CODE = '".$sections[$k]['RATING_CYCLE_CODE']."'";
				mysql_query($query) or die('mysql_error: '.mysql_error());
